...
from pharmadiff.utils import NoSyncMetric as Metric, NoSyncMetricCollection as MetricCollection
from pharmadiff.analysis.rdkit_functions import check_stability
from pharmadiff.utils import NoSyncMAE as MeanAbsoluteError
...
class SamplingMetrics(nn.Module):
    def __init__(self, train_smiles, dataset_infos, test):
        ...
        #self.conf_pharma_match = MeanMetric()
        self.rdkit_pharma_match = MeanMetric()
        self.rdkit_SAS = MeanMetric()
        self.rdkit_QED = MeanMetric()
        self.ring_filter = MeanMetric()
        self.pains_filter = MeanMetric()
        self.pass_2d_filter = MeanMetric()
        self.n_rings = MeanMetric()
        self.pgmg_match_score = MeanMetric()
        self.pgmg_valid_match_score = MeanMetric()
        self.pgmg_percentage = MeanMetric()
        self.pgmg_abobe_threshold = MeanMetric()
    ...
    def reset(self):
        for metric in [self.atom_stable, self.mol_stable, self.validity_metric, self.uniqueness,
                       ...
                       self.bond_lengths_w1, self.angles_w1, self.rdkit_pharma_match,
                       self.rdkit_SAS, self.pgmg_match_score, self.pgmg_percentage, 
        ...
        for mol in generated:
            ...
            if rdmol is not None:
                try:
                    ...
                    valid_rdkit_mols.append(largest_mol)
    ...
    def evaluate(self, generated, local_rank):
        ...
        # Validity
        valid, valid_rdkit_mols, valid_mols,connected_components, all_smiles, error_message = self.compute_validity(generated)
        ...
        if rdkit_metrics:       
            ...
            print(f"rdkit SAS: {rdkit_SAS:.2f}")
            print(f"rdkit QED: {rdkit_QED:.2f}")
            print(f'Ring filter: {ring_filter:.2f}')
            print(f'PAINS filter: {pains_filter:.2f}')
            print(f'PASS 2D filter: {pass_2d_filter :.2f}')
            print(f'Number of rings: {n_rings:.2f}')
            print(f"pgmg pharmacophore match: {pgmg_match_score * 100 :.2f}%")
            print(f"pgmg pharmacophore match percentage: {percentage_pgmg * 100 :.2f}%")
            print(f"pgmg pharmacophore match above 0.8: {pgmg_above_threshold * 100 :.2f}%")
            print(f"pgmg valid pharmacophore match: {pgmg_valid_match_score * 100 :.2f}%")
            ...
            if wandb.run:
                dic = {
                    'rdkit_pharma_match': rdkit_pharma_match,
                    'rdkit_SAS': rdkit_SAS, 
                    'rdkit_QED': rdkit_QED,
                    'ring_filter': ring_filter,
                    'pains_filter': pains_filter,
                    'pass_2d_filter': pass_2d_filter,
                    'n_rings': n_rings,
    ...
    def rdkit_pharma_satisfaction(self, molecules):
        ...
        for i, mol in enumerate(molecules):
            ...
            if rdkit_mol is None:
                #print(f"rdkit_mol is None")
                satisfied_percentage.append(-1)
                continue
            ...
            length = len(ref_labels)  # Get the length of the current ref_labels
            ...
            match_3d = match_mol(rdkit_mol, ref_labels, ref_coords)
            satisfied_percentage.append(match_3d)
    ...
    def rdkit_valid_pharma_satisfaction(self, valid_molecules, valid_rdkit_mols):
        ...
        for i, mol in enumerate(valid_molecules):
            ...
            if mol.rdkit_mol is None:
                ...
                match_3d = match_mol(valid_rdkit_mols[i], ref_labels, ref_coords)
                satisfied_percentage.append(match_3d)
        ...
        try:
            self.rdkit_valid_pharma_match.update(value= sum(valid_match_score) /len(valid_match_score),
                                                weight=len(valid_match_score))

        except:
            self.rdkit_valid_pharma_match.update(value= 0, weight=1)
        
    
    def rdkit_sas_and_qed(self, molecules):
        sas = []
        qeds = []
        ring_filter = []
        pains_filter = []
        ...
        for i, mol in enumerate(molecules):
            if mol is None:
                sas.append(-1)
                qeds.append(-1)
                ring_filter.append(-1)
                pains_filter.append(-1)
                pass_2d_filter.append(-1)
                number_rings.append(-1)
                ...
                try:
                    ...
                    qed = -1
                try:
                    ra = check_ring_filter(mol)
                except Exception as e:
                    # print(f'Could not compute ring filter: {e}')
                    ra = -1
                try:
                    pf = check_pains(mol, pains_smarts)
                except Exception as e:
                    # print(f'Could not compute PAINS filter: {e}')
                    pf = -1
                try:
                    nr = Chem.rdMolDescriptors.CalcNumRings(mol)
                except Exception as e:
                    # print(f'Could not compute number of rings: {e}')
                    nr = -1
                ...
                sas.append(sa)
                qeds.append(qed)
                ring_filter.append(ra)
                pains_filter.append(pf)
                pass_2d_filter.append(pass_2d)
                number_rings.append(nr)
                
        sas = [i for i in sas if i >= 0]
        qeds = [i for i in qeds if i >= 0]
        ring_filter = [i for i in ring_filter if i >= 0]
        pains_filter = [i for i in pains_filter if i >= 0]
    ...
    def pgmg_match_score_fn(self, molecules):
        ...
        for i, mol in enumerate(molecules):
            ...
            if rdkit_mol is None:
                satisfied_percentage.append(-1)
                continue
            if mol.match_score is not None:
                satisfied_percentage.append(mol.match_score)
                continue
            ...
            np_ref_labels = ref_labels.detach().cpu().numpy().reshape(-1, 1).astype(int)
            np_ref_coords = ref_coords.detach().cpu().numpy()
            pgmg_match = match_score(rdkit_mol , np_ref_labels, np_ref_coords)
            #print(f"pgmg match: {pgmg_match}")
            
            satisfied_percentage.append(pgmg_match)
    ...
    def pgmg_valid_match_score_fn(self, valid_molecules, valid_rdkit_mols):
        ...
        for i, mol in enumerate(valid_molecules):
            ...
            np_ref_labels = ref_labels.detach().cpu().numpy().reshape(-1, 1).astype(int)
            np_ref_coords = ref_coords.detach().cpu().numpy()
            pgmg_match = match_score(valid_rdkit_mols[i] , np_ref_labels, np_ref_coords)
            satisfied_percentage.append(pgmg_match)
    ...
    def save_sdf(self, molecules):
        file_name = 'generated_molecules.sdf'
    
        # Open writer in append mode if file exists, else create new
        ...
        writer.close()
        print(f'Molecules written to {file_name}') 
...
if __name__ == '__main__':
    ...
    from pharmadiff.utils import setup_wandb
